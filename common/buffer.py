...
class ReplayBuffer:
    def __init__(
        ...
        self.risks = np.zeros((self.max_size, 1), dtype=np.float32)  # 新增risk字段

    def add(self, obs, next_obs, action, reward, terminal, risk=0.0):
        ...
        self.risks[self.ptr] = np.array(risk).copy()  # 新增risk
    ...
    def load_dataset(self, dataset):
        ...
        # 对于离线数据，risk都为0
        risks = np.zeros((len(observations), 1), dtype=np.float32)
        ...
        self.risks = risks  # 新增risk

        self.ptr = len(observations)
        self.size = len(observations)

    def add_batch(self, obs, next_obs, actions, rewards, terminals, risks=None):
        batch_size = len(obs)
        if risks is None:
            risks = np.zeros((batch_size, 1), dtype=np.float32)
        ...
        if self.ptr + batch_size > self.max_size:
            ...
            self.risks[begin:end] = np.array(risks[:first_add_size]).copy()  # 新增risk
            ...
            self.risks[begin:end] = np.array(risks).copy()  # 新增risk
    ...
    def sample(self, batch_size):
        ...
        return {
            ...
            "risks": self.risks[batch_indices].copy()  # 新增risk
        }
    ...
    def sample_all(self):
        return {
            ...
            "risks": self.risks[:self.size].copy()  # 新增risk
        }
