...
class MOPO:
    def __init__(
        ...
        self.low_risk_weight = 0.2
        self.high_risk_weight = 2


    def _sample_initial_transitions(self):
        return self.offline_buffer.sample(self._rollout_batch_size)

    def _adaptive_conservative_weights(self, risk_mean, risk_var):
        p = risk_mean / (risk_mean + risk_var)
        self.policy.risk_alpha = self.low_risk_weight + (self.high_risk_weight - self.low_risk_weight) * p
        print(self.policy.risk_alpha)
    ...
    def rollout_transitions(self):
        ...
        for _ in range(self._rollout_length):
            ...
            next_observations, rewards, terminals, risks,infos = self.dynamics_model.predict(observations, actions)
            # 修改: 传递risk到buffer
            #risks = infos['risk']
            self.model_buffer.add_batch(observations, next_observations, actions, rewards, terminals, risks)
            if self.use_adapt:
                # 累加 risk 的均值和方差
                risk_sum += risks.sum()
                risk_sq_sum += (risks ** 2).sum()
        ...
        if self.use_adapt:
            ...
            if risk_count > 0:
                ...
                risk_var = (risk_sq_sum / risk_count) - (risk_mean ** 2)
                self._adaptive_conservative_weights(risk_mean, risk_var)
    ...
    def learn_policy(self):
        ...
        data = {
            ...
            "risks": np.concatenate([real_batch["risks"], fake_batch["risks"]], axis=0)
        }
