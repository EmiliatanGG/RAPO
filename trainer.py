...
class Trainer:
    ...
    def train_policy(self):
        ...
        for e in range(1, self._epoch + 1):
            ...
            all_return.append([ep_reward_mean,ep_reward_std])#,self.algo.policy.risk_alpha
            #print(self.algo.policy.risk_alpha)
        
            # save policy
            torch.save(self.algo.policy.state_dict(), os.path.join(self.logger.writer.get_logdir(), "policy.pth"))
        self.logger.print("total time: {:.3f}s".format(time.time() - start_time))
        with open('halfmr_0_nomodel.txt', 'w', encoding='utf-8') as f:
            for item in all_return:
                f.write(f"{item}\n")
