    ...
    def predict(self, obs, act, deterministic=False):
        ...
        if penalty_coeff != 0:
            if not penalty_learned_var:
                ...
                risk = np.max(dists, axis=0)  # max distances over models
            else:
                risk = np.amax(np.linalg.norm(ensemble_model_stds, axis=2), axis=0)
        else:
            risk = np.zeros(batch_size)

        # 不再对奖励进行惩罚，直接使用原始奖励
        # risk将作为单独的字段存储在buffer中

        assert (type(next_obs) == np.ndarray)
        info = {'risk': risk, 'raw_rewards': rewards}
        rewards = rewards[:, None]
        risk=risk[:, None]
        terminals = terminals[:, None]
        return next_obs, rewards, terminals, risk,info
